...
Include `delivery_id`, issue reference, session ID, source branch, source SHA, startup
...
complete for the same SHA. If evidence cannot be produced, return `blocked` with the
