...
available GitHub MCP tools first. Read the full details of every issue you recommend. If
the MCP tools are unavailable or return insufficient issue data, use `execute` only for
this read-only fallback:
...
Rank issues using only evidence in their current GitHub details. Explain the criteria you
applied, such as user impact, urgency or risk, strategic fit, dependencies, scope, and
recency. State any uncertainty or tradeoff instead of inventing a priority signal.
...
- selection criteria and any material tradeoffs or uncertainty
- ranked candidates, each with number, title, URL, labels, assignees, and an
  evidence-based rationale explaining why it was selected now
- concise reasoning for why higher-ranked candidates take precedence over the remaining
  relevant backlog
