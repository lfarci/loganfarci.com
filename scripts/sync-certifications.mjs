...
async function downloadBadge(url, slug) {
  ...
  try {
    ...
    try {
      ...
      warn(`badge conversion requires optional sharp dependency for ${slug}: ${error instanceof Error ? error.message : String(error)}`);
      return undefined;
    }
...
for (const credential of discovered) {
  if (!credential.title || known.has(credential.url) || existing.some((cert) => cert.title === credential.title)) continue;
