...
export const motionDurations = { page: 0.34 } as const;

export const naturalEase: [number, number, number, number] = [0.22, 1, 0.36, 1];

export const pageTransition: Transition = { duration: motionDurations.page, ease: naturalEase };

export const reducedMotionTransition: Transition = { duration: 0 };

export const pageTransitionVariants: Variants = {
    enter: (direction: RouteMotionDirection = 0) => ({ opacity: 0, x: direction * 16 }),
    center: { opacity: 1, x: 0 },
    exit: (direction: RouteMotionDirection = 0) => ({ opacity: 0, x: direction * -12 }),
};
